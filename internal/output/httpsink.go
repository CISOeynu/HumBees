...
type HTTPConfig struct {
	...
	AllowInsecure bool
	// TLSSkipVerify disables certificate verification for https:// targets.
	// Use this only when pointing at a server with a self-signed cert on a
	// trusted network (e.g. a local bumblebee-server instance during testing).
	// Transport is still TLS-encrypted; only the identity check is skipped.
	TLSSkipVerify bool
	...
	if client == nil {
		...
		if cfg.TLSSkipVerify {
			transport = &http.Transport{
				TLSClientConfig: &tls.Config{InsecureSkipVerify: true}, //nolint:gosec // intentional, operator-requested
			}
		}
